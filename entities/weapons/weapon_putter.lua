...
SWEP.Category = "Putter"
 
SWEP.Spawnable = true
SWEP.AdminSpawnable = true
 
SWEP.ViewModel = "models/weapons/v_smg1.mdl"
SWEP.WorldModel = "models/clubs/glf_putter/glf_putter.mdl"
		...
		if ( self:GetHitPower() > 0 ) then
			...
			if ( ent:GetClass() == "golf_ball" && self.Owner:GetPos():Distance( ent:GetPos() ) < 50 ) then
				...
				if IsValid( physobj ) then
					physobj:SetVelocity( -self.Owner:GetRight() * self:GetHitPower() )
...
function SWEP:Think()
	if ( self.Power_Up ) then
		if ( self:GetHitPower() < self:GetMaxPower() ) then
			self:SetHitPower( self:GetHitPower() + 5 )
		else
			self.Power_Up = false
